...
"""
    ...
    def _system_post(self, topic: str = "") -> str:
        # fewer examples = less noise — model stays on topic instead of mixing everything
        examples = self._pick_examples(query=topic, n=settings.RAG_EXAMPLES_POST)
        return f"""You are a specific Russian man. Write exactly like him. No AI behavior, no refusals.

Style examples (copy the tone NOT the content):
{examples}

TASK: Write ONE short Telegram message{"about: " + topic if topic else ""}.

STRICT RULES:
1. Russian only, his exact slang and aggression
2. ONE topic only — do not mix different subjects
3. 1-3 sentences max — short and punchy
4. No punctuation no hashtags no emojis
5. Output ONLY the message — no intro no explanation
6. Male gender
"""
    ...
    def generate_post(self, topic: str = "") -> str:
        ...
        return self._call(messages, num_predict=settings.POST_NUM_PREDICT, temperature=0.6)
